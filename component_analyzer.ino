/* 
 ...
 COMPONENT ANALYZER v0.1b 
 25/08/2014
 
 
 CHANGELOG:
   08/04/2015 - Rebuilt from random arduino component tester source.
   08/04/2015 - Cleaning up source and version bump
 
 TODO:
  - Fix button press loop
...
#define COMPARATOR_OFFSET    15                   //15
...
#else
#define pushButton            A3                 //PushButton - Set A3 if you non use a I2C LCD
#endif
...
const char Failed6_str[] PROGMEM = "Self Test Failed";
const char Induct_str[] PROGMEM = "Inductance";
...
const char Version_str[] PROGMEM = "v0.1b";
const char NPN_str[] = "NPN";
const char PNP_str[] = "PNP";
const char URef_str[] PROGMEM = "Vref";
const char Rl_str[] PROGMEM = "+l-";
const char Rh_str[] PROGMEM = "+h-";
const char ProbeComb_str[] PROGMEM = "12 13 23";
const char RiLow_str[] PROGMEM = "i-";
const char RiHigh_str[] PROGMEM = "i+";
const char RhLow_str[] PROGMEM = "h-";
const char RhHigh_str[] PROGMEM = "h+";
const char ROffset_str[] PROGMEM = "R0";
const char CapOffset_str[] PROGMEM = "C0";
const char CompOffset_str[] PROGMEM = "AComp";
const char ShortCircuit_str[] PROGMEM = "Short Circuit!";
const char IsolateProbe_str[] PROGMEM = "Isolate Probe!";
const char Shortprob_str[] PROGMEM = "Short Probe!";
...
//  lcd.home();                                  //Doesn't seem to be required
  lcd.print(F("Component "));
  lcd.print(Y(Version_str));
  lcd_line(2);
  lcd.print(F("Analyzer"));
...
void loop() {  
  ...
  if (AllProbesShorted() == 3) {
    ...
    if ((CompFound == COMP_NONE) || (CompFound == COMP_RESISTOR) || (CompFound == COMP_DIODE)) {
...
#ifdef DEBUG_PRINT
    Serial.println(F(""));  
    Serial.print(F("Component Found:")); 
...
//probe up/down via probe resistor for 1 or 10 ms
...
void CheckDiode(void)
{
  ...
  delay(5);
  U1_Rl = ReadU(Probe1_Pin);                      //Get voltage at anode 
  ...
  PullProbe(Probe3_Rl, FLAG_10MS | FLAG_PULLUP);  //Discharge gate 
  delay(5);
  U1_Rh = ReadU(Probe1_Pin);                      //Get voltage at anode, neglect voltage at cathode
  ...
  delay(5);
  U2_Rl = ReadU(Probe1_Pin);                      //Get voltage at anode 
  ...
  PullProbe(Probe3_Rl, FLAG_10MS | FLAG_PULLDOWN);//Discharge gate 
  delay(5);
  U2_Rh = ReadU(Probe1_Pin);                      //Get voltage at anode, neglect voltage at cathode
...
void CheckResistor(void)
{
  ...
  if (U_Rh_L <= 20)
  {
    ...
    R_PORT = Probe1_Rh;                           //Pull up probe-1 via Rh
    delay(5);
    U_Rh_H = ReadU(Probe1_Pin);                   //Get voltage at Rh pulled up
    ...
    delay(5);
    U_Ri_H = ReadU(Probe1_Pin);                   //Get voltage at internal R  
    ...
    delay(5);
    U_Rh_L = ReadU(Probe2_Pin);                   //Get voltage at Rh pulled down 
...
unsigned int CheckDepModeFET(void)
{
  ...
  delay(5);
  U_Rl_L = ReadU(Probe2_Pin);                     //Get voltage at source
  ...
  if (U_Rl_L >= 977)                              //This might by a p-channel
  {
    ...
    delay(5);
    U_Rl_L = ReadU(Probe2_Pin);                   //Get voltage at drain
  }
  ...
  if (U_Rl_L > 92)
  {
    ...
    if (CompDone == 0)                            //No component found yet
    {
      ...
      delay(20);
      U_1 = ReadU(Probe2_Pin);                    //Voltage at source

      R_PORT = Probe3_Rh;                         //Pull up gate via Rh 
      delay(20);
      U_2 = ReadU(Probe2_Pin);                    //Voltage at source 
      if (U_2 > (U_1 + 488))
      {
        ...
        delay(20);
        U_2 = ReadU(Probe3_Pin);                  //Get voltage at gate
        if (U_2 > 3911)                           //MOSFET
        {
    ...
    if (CompDone == 0)                            //No component found yet
    {
      ...
      delay(20);
      U_1 = ReadU(Probe1_Pin);                    //Get voltage at source
      R_PORT = Probe1_Rl;                         //Pull down gate via Rh
      delay(20);
      U_2 = ReadU(Probe1_Pin);                    //Get voltage at source
      if (U_1 > (U_2 + 488))
      {
        //Compare gate voltage to distinguish JFET from MOSFET
...
unsigned long Get_hfe_c(byte Type)
{
  ...
  if (Type == TYPE_NPN)                           //NPN
  {
    ...
    delay(5);
    U_R_e = ReadU(Probe2_Pin);                    //U_R_e = U_e 
  ...
  else                                            //PNP 
  {
    ...
    delay(5);
    U_R_e = UREF_VCC - ReadU(Probe1_Pin);         //U_R_e = Vcc - U_e 
...
byte CheckThyristorTriac(void)
{
  ...
  PullProbe(Probe3_Rl, FLAG_10MS | FLAG_PULLDOWN);//Discharge gate
  delay(5);
  U_1 = ReadU(Probe1_Pin);                        //Get voltage at anode
  R_PORT = 0;                                     //Pull down anode
  delay(5);
  R_PORT = Probe1_Rl;                             //and pull up anode again
  delay(5);
  U_2 = ReadU(Probe1_Pin);                        //Get voltage at anode (below Rl)
  ...
  if ((U_1 < 1600) && (U_2 > 4400))
  {
    ...
    delay(5);
    U_1 = ReadU(Probe1_Pin);                      //Get voltage at MT2 
    ...
    if (U_1 <= 244)
    {
      ...
      delay(5);
      U_1 = ReadU(Probe3_Pin);                    //Get voltage at gate 
...
void CheckBJTorDepMOSFET(byte BJT_Type, unsigned int U_Rl)
{
  ...
  else                                            //PNP
  {
    ...
    delay(5);
    U_R_c = ReadU(Probe2_Pin);                    //U_R_c = U_c
    U_R_b = ReadU(Probe3_Pin);                    //U_R_b = U_b
  }
  ...
  if (U_R_b > BJT_Level)                          //U_R_b matches minimum level of BJT
  {
    ...
    if (BJT_Type == TYPE_NPN)                     //NPN
    {
      ...
      delay(5);
      U_R_b = UREF_VCC - ReadU(Probe2_Pin);       //U_R_c = Vcc - U_c        
    }
    else                                          //PNP
    { 
      //We assume: probe-1 = C / probe-2 = E / probe-3 = B 
      ...
      delay(5);
      U_R_b = ReadU(Probe1_Pin);                  //U_R_c = U_c 
    }
...
void CheckProbes(byte Probe1, byte Probe2, byte Probe3)
{
  ...
  if (U_Rl < 977)                                 //Load current < 1.4mA
  {
    ...
    if (CompDone == 0)                           //Not sure yet
    {
      ...
      delay(5);
      U_1 = ReadU(Probe2);                       //Get voltage at collector  
    ...
    if (CompDone == 0)                           //Not sure yet
    {
      ...
      delay(5);
      U_1 = ReadU(Probe1);                       //Get voltage at collector 
...
void LoadAdjust(void)
{
  ...
  #ifdef LCD_I2C
  DIDR0 = 0b00111111;			         //DIDR0 â€“ Digital Input Disable Register 0 - Disable digital input on analog pins
  #else
  DIDR0 = 0b00110111;
  #endif
}
...
//Check short-circuit of all three probes 
...
byte SelfTest(void)
{
  ...
  {
    Counter = 1;
    //Rrepeat each test 5 times
    while (Counter <= 5)
    {
      //Display test number
      lcd_clear();
      lcd_data('T');                              //Test number                        
      lcd_data('0' + Test); 
      lcd_send(Counter+64);      
      lcd_space();
      DisplayFlag = 1;                 
      ...
      case 3:                                   //Compare Rh resistors (probes still shorted)
        lcd_print(Y(Rh_str));                   //Display: +Rh-
        lcd_space();
        lcd_print(Y(ProbeComb_str));            //Display: 12 13 23
        //Set up a voltage divider with the Rh's - TP1: Gnd -- Rh -- probe-2 -- probe-1 -- Rh -- Vcc
        R_PORT = 2 << (TP1 * 2);
      ...
      case 6:                                   //Rh resistors pulled up 
        lcd_print(Y(RhHigh_str));               //Display: Rh+ 
        //TP1: probe -- Rh -- Vcc
        R_DDR = 2 << (TP1 * 2);
        R_PORT = 2 << (TP1 * 2);
        delay(20);
...
//Interpolate a table with the function -log(1 - (permil/1000))
unsigned int get_log(unsigned int permil) {
#define Log_Tab_Distance 20                     //Displacement of table is 20 mil
  unsigned int      y1, y2;		          //Table values
  unsigned int      result;	                  //Result of interpolation
  byte              tabind;	                  //Index to table value
  byte              tabres;                       //Distance to lower table value, fraction of Log_Tab_Distance
  ...
  y1 = pgm_read_word(&Log_table[tabind]);         //Get the lower table value
  y2 = pgm_read_word(&Log_table[tabind+1]);       //Get the higher table value
  //Interpolate
...
byte SelfAdjust(void)
{
  ...
  //Measurements
  lcd_clear();
  lcd_print(Y(Shortprob_str));                    //Display: Short probes! 
  ...
  while (Test <= 5)
  {
    ...
    while (Counter <= 5)
    {
      ...
      case 1:                                   //Resistance of probe leads (probes shorted) 
        lcd_print(Y(ROffset_str));              //Display: R0 
        lcd_space();
        lcd_print(Y(ProbeComb_str));            //Display: 12 13 23          
      ...
      case 3:                                   //Internal resistance of ÂµC in pull-down mode
        lcd_print(Y(RiLow_str));                //Display: Ri-
      ...
      case 5:                                   //Capacitance offset (PCB and probe leads)
        lcd_print(Y(CapOffset_str));            //Display: C0
        lcd_space();
        lcd_print(Y(ProbeComb_str));            //Display: 12 13 23
  ...
  //Calculate values and offsets, capacitance auto-zero
  if (CapCounter == 15)
  {
...
void ShowAdjust(void)
{
  ...
  lcd_line(2);
  lcd_print(Y(RiHigh_str));                       //Display: Ri+ 
  lcd_space();
...
#else
  delay(3000);                                  //Wait 3sec  
#endif  //Display offset of bandgap reference
  lcd_clear();
  lcd_print(Y(URef_str));                         //Display: Vref
  lcd_space();
